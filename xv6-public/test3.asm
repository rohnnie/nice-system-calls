...
int main(int argc, char *argv[]) {
   ...
   9:	83 ec 20             	sub    $0x20,%esp
    int priorities[] = {2, 2, 4,4};  // Example priorities for each process
   c:	c7 44 24 10 02 00 00 	movl   $0x2,0x10(%esp)
  13:	00 
  14:	c7 44 24 14 02 00 00 	movl   $0x2,0x14(%esp)
  1b:	00 
  1c:	c7 44 24 18 04 00 00 	movl   $0x4,0x18(%esp)
  23:	00 
  24:	c7 44 24 1c 04 00 00 	movl   $0x4,0x1c(%esp)
  2b:	00 
    int process_count = sizeof(priorities) / sizeof(priorities[0]);
    int t = 100; // Time threshold in ticks
    int i;
    for (i = 0; i < process_count; i++) {
        if (fork() == 0) {
  2c:	e8 19 03 00 00       	call   34a <fork>
  31:	85 c0                	test   %eax,%eax
  33:	74 25                	je     5a <main+0x5a>
int main(int argc, char *argv[]) {
    int priorities[] = {2, 2, 4,4};  // Example priorities for each process
    int process_count = sizeof(priorities) / sizeof(priorities[0]);
    int t = 100; // Time threshold in ticks
  ...
  38:	83 fb 04             	cmp    $0x4,%ebx
  3b:	75 ef                	jne    2c <main+0x2c>
        }
    }
    ...
    for (i = 0; i < process_count; i++) {
        wait();
  3d:	e8 18 03 00 00       	call   35a <wait>
  42:	e8 13 03 00 00       	call   35a <wait>
  47:	e8 0e 03 00 00       	call   35a <wait>
  ...
  5a:	c7 44 24 04 64 00 00 	movl   $0x64,0x4(%esp)
  61:	00 
  62:	8b 44 9c 10          	mov    0x10(%esp,%ebx,4),%eax
  66:	89 04 24             	mov    %eax,(%esp)
  69:	e8 02 00 00 00       	call   70 <calculate_primes>
  6e:	66 90                	xchg   %ax,%ax

00000070 <calculate_primes>:
#include "types.h"
#include "stat.h"
#include "user.h"
...
// Function to calculate and print prime numbers up to 1000
void calculate_primes(int priority, int t) {
  7d:	83 ec 2c             	sub    $0x2c,%esp
    int pid = getpid();
  80:	e8 4d 03 00 00       	call   3d2 <getpid>
  ...
  91:	e8 64 03 00 00       	call   3fa <nice>
    int i,j;
    int start_time = uptime();
  96:	e8 4f 03 00 00       	call   3ea <uptime>
    int prime_count = 0;

    printf(1, "Started Current PID: %d, Priority: %d\n", pid, priority);
  9b:	89 74 24 08          	mov    %esi,0x8(%esp)
  9f:	c7 44 24 04 18 08 00 	movl   $0x818,0x4(%esp)
  a6:	00 
...
void calculate_primes(int priority, int t) {
  ...
  b4:	89 44 24 0c          	mov    %eax,0xc(%esp)
  b8:	e8 f3 03 00 00       	call   4b0 <printf>
  bd:	8d 76 00             	lea    0x0(%esi),%esi
    ...
    for (i = 2; i <= 10000000; i++) {
        int is_prime = 1;
        for (j = 2; j * j <= i; j++) {
  c0:	83 fb 03             	cmp    $0x3,%ebx
  c3:	7e 28                	jle    ed <calculate_primes+0x7d>
            if (i % j == 0) {
  c5:	f6 c3 01             	test   $0x1,%bl
  c8:	74 26                	je     f0 <calculate_primes+0x80>
  ca:	b9 02 00 00 00       	mov    $0x2,%ecx
  cf:	eb 10                	jmp    e1 <calculate_primes+0x71>
  d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  d8:	89 d8                	mov    %ebx,%eax
  da:	99                   	cltd   
  db:	f7 f9                	idiv   %ecx
  dd:	85 d2                	test   %edx,%edx
  df:	74 0f                	je     f0 <calculate_primes+0x80>
    printf(1, "Started Current PID: %d, Priority: %d\n", pid, priority);
    ...
    for (i = 2; i <= 10000000; i++) {
        int is_prime = 1;
        for (j = 2; j * j <= i; j++) {
  e1:	83 c1 01             	add    $0x1,%ecx
  e4:	89 c8                	mov    %ecx,%eax
  e6:	0f af c1             	imul   %ecx,%eax
  e9:	39 d8                	cmp    %ebx,%eax
  eb:	7e eb                	jle    d8 <calculate_primes+0x68>
                is_prime = 0;
                break;
            }
        }
  ...
  f3:	81 fb 81 96 98 00    	cmp    $0x989681,%ebx
  f9:	75 c5                	jne    c0 <calculate_primes+0x50>
        if (is_prime) {
            prime_count++;
        }
    }
    ...
    int end_time = uptime();
  fb:	e8 ea 02 00 00       	call   3ea <uptime>
 ...
 100:	89 7c 24 14          	mov    %edi,0x14(%esp)
 104:	89 74 24 08          	mov    %esi,0x8(%esp)
            prime_count++;
        }
    }
    ...
    int end_time = uptime();
    int execution_time = end_time - start_time;
 108:	2b 45 e4             	sub    -0x1c(%ebp),%eax
    
    printf(1, "Finished Current PID: %d, Priority: %d, Execution Time: %d ticks, Total Primes Found: %d\n", pid, priority, execution_time, prime_count);
 10b:	c7 44 24 04 40 08 00 	movl   $0x840,0x4(%esp)
 112:	00 
 113:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 11a:	89 44 24 10          	mov    %eax,0x10(%esp)
 11e:	8b 45 08             	mov    0x8(%ebp),%eax
 121:	89 44 24 0c          	mov    %eax,0xc(%esp)
 125:	e8 86 03 00 00       	call   4b0 <printf>

    exit();
 12a:	e8 23 02 00 00       	call   352 <exit>
 12f:	90                   	nop
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 449:	0f b6 92 a3 08 00 00 	movzbl 0x8a3(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5e1:	b8 9c 08 00 00       	mov    $0x89c,%eax
...
free(void *ap)
{
 ...
 6a1:	a1 3c 0b 00 00       	mov    0xb3c,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 6e7:	89 10                	mov    %edx,(%eax)
  freep = p;
 6e9:	a3 3c 0b 00 00       	mov    %eax,0xb3c
}
 6ee:	5b                   	pop    %ebx
 6ef:	5e                   	pop    %esi
 6f0:	5f                   	pop    %edi
...
malloc(uint nbytes)
{
 ...
 739:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 73c:	8b 1d 3c 0b 00 00    	mov    0xb3c,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 779:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 77b:	3b 15 3c 0b 00 00    	cmp    0xb3c,%edx
...
morecore(uint nu)
{
 ...
 7ad:	e8 ee fe ff ff       	call   6a0 <free>
  return freep;
 7b2:	8b 15 3c 0b 00 00    	mov    0xb3c,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7ec:	eb e9                	jmp    7d7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ee:	c7 05 3c 0b 00 00 40 	movl   $0xb40,0xb3c
 7f5:	0b 00 00 
    base.s.size = 0;
 7f8:	ba 40 0b 00 00       	mov    $0xb40,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7fd:	c7 05 40 0b 00 00 40 	movl   $0xb40,0xb40
 804:	0b 00 00 
    base.s.size = 0;
 807:	c7 05 44 0b 00 00 00 	movl   $0x0,0xb44
 80e:	00 00 00 
